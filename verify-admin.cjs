...
(async () => {
  ...
  await page.goto('file:///'+path.join(__dirname,'site_template.html').replaceAll('\\','/')+'#admin');
  ...
  assert.match(await page.locator('html').evaluate(e=>e.style.getPropertyValue('--font-display')),/Georgia/);
  await page.reload();
  await page.locator('[data-tab="design"]').click();
  ...
  await nested.evaluate(e=>{let p=e.parentElement;while(p){if(p.tagName==='DETAILS')p.open=true;p=p.parentElement;}});
  ...
  await page.screenshot({path:path.join(__dirname,'admin-desktop.png'),fullPage:false});
  await page.locator('[data-tab="tools"]').click();
  const downloadPromise=page.waitForEvent('download');
  await page.locator('[data-action="download-site"]').click();
  const download=await downloadPromise;
  const downloaded=path.join(__dirname,'exported-preview.html');
  ...
  assert.match(await exportedPage.locator('html').evaluate(e=>e.style.getPropertyValue('--font-display')),/Georgia/);
  await page.evaluate(()=>location.hash='#home');
  await page.waitForTimeout(200);
  assert.equal(await page.locator('#about').isVisible(),false);
  assert.equal(await page.locator('.btn').first().evaluate(e=>getComputedStyle(e).borderRadius),'0px');
  await page.setViewportSize({width:390,height:844});
  await page.screenshot({path:path.join(__dirname,'site-mobile.png'),fullPage:false});
  ...
  await browser.close();
  console.log('PASS: typography, nested content, focus, persistence, visibility, export, mobile overflow, runtime errors');
